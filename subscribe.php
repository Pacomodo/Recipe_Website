...
$subscribe_user_id = isset($_GET['user_id']) ? trim($_GET['user_id']) : '';

if ($subscribe_user_id) {
    // 구독 추가
    $stmt = $conn->prepare("INSERT INTO subscribe (user_id, subscribe_id) VALUES (?, ?)");
    if ($stmt === false) {
    ...
    if ($stmt->execute()) {
        ...
        $message = "구독 중 오류가 발생했습니다. 다시 시도해 주세요.";
    }
} else {
...
<body>
    ...
    <main>
        ...
        <a href="subscriptions.php">돌아가기</a>
    </main>
